...
export function MarketDashboardV2({
    ...
    return (
        <div className="min-h-screen bg-background p-4 md:p-6 space-y-6 text-foreground font-sans">
            ...
            {/* Header Section */}
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 sticky top-0 z-30 bg-background/80 backdrop-blur-md py-2 -mx-2 px-2 transition-all">
                <div>
                ...
                <div className="flex items-center gap-3">
                    <div className={cn(
                        ...
                        <span className="relative flex h-2 w-2">
                            <span className={cn("animate-ping absolute inline-flex h-full w-full rounded-full opacity-75", isConnected ? "bg-success" : "bg-destructive")}></span>
                            <span className={cn("relative inline-flex rounded-full h-2 w-2", isConnected ? "bg-success" : "bg-destructive")}></span>
                        </span>
            ...
            >
                <CleanCard className="p-4 flex items-center justify-between hover:shadow-md transition-shadow cursor-default group">
                    <div>
                        <p className="text-muted-foreground text-xs uppercase font-bold tracking-wider">Tổng khối lượng</p>
                ...
                <CleanCard className="p-4 flex items-center justify-between hover:shadow-md transition-shadow cursor-default group">
                    <div>
                        <p className="text-muted-foreground text-xs uppercase font-bold tracking-wider">Mã tăng</p>
                        <p className="text-lg md:text-xl font-bold text-success mt-1">
                            {marketStats?.advancing || 0}
                        </p>
                    </div>
                    <div className="p-2 bg-success/10 rounded-lg text-success group-hover:bg-success/20 transition-colors">
                ...
                <CleanCard className="p-4 flex items-center justify-between hover:shadow-md transition-shadow cursor-default group">
                    <div>
                        <p className="text-muted-foreground text-xs uppercase font-bold tracking-wider">Mã giảm</p>
                        <p className="text-lg md:text-xl font-bold text-destructive mt-1">
                            {marketStats?.declining || 0}
                        </p>
                    </div>
                    <div className="p-2 bg-destructive/10 rounded-lg text-destructive group-hover:bg-destructive/20 transition-colors">
                ...
                <CleanCard className="p-4 flex items-center justify-between hover:shadow-md transition-shadow cursor-default group">
                    <div>
                        <p className="text-muted-foreground text-xs uppercase font-bold tracking-wider">Tham chiếu</p>
                        <p className="text-lg md:text-xl font-bold text-warning mt-1">
                            {marketStats?.unchanged || 0}
                        </p>
                    </div>
                    <div className="p-2 bg-warning/10 rounded-lg text-warning group-hover:bg-warning/20 transition-colors">
            ...
            <div className="grid grid-cols-12 gap-6 h-auto">
                ...
                <div className="col-span-12 lg:col-span-3 space-y-6">
                    <div className="sticky top-20 space-y-6">
                        ...
                        <CleanCard className="p-4 bg-card">
                            <h3 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
                                <Monitor className="w-4 h-4 text-primary" /> Trạng thái thị trường
                            </h3>
                            <div className="space-y-3 text-sm">
                                <div className="flex justify-between items-center">
                                    <span className="text-muted-foreground">Phiên</span>
                                    <span className="font-medium text-success bg-success/10 px-2 py-0.5 rounded text-xs">Mở cửa</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-muted-foreground">Trạng thái</span>
                                    <span className="font-medium text-foreground">Khớp lệnh liên tục</span>
                                </div>
                                <div className="h-px bg-border my-2" />
                                <div className="flex justify-between items-center">
                                    <span className="text-muted-foreground">Tổng mã CP</span>
                                    <span className="font-mono font-medium text-foreground">{marketStats?.totalStocks || 0}</span>
                                </div>
                            </div>
                        </CleanCard>
                    </div>
