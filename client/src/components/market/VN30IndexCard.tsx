...
export default function VN30IndexCard({
    ...
    const getBgGradient = () => {
        if (isPositive) return 'from-emerald-900/20 to-transparent';
        if (isNegative) return 'from-red-900/20 to-transparent';
        return 'from-amber-900/20 to-transparent';
    };

    const getIconBg = () => {
        if (isPositive) return 'bg-emerald-500/20';
        if (isNegative) return 'bg-red-500/20';
    ...
    const getChangeBadge = () => {
        if (isPositive) return 'bg-emerald-500/20 text-emerald-400';
        if (isNegative) return 'bg-red-500/20 text-red-400';
    ...
    return (
        <div className="relative group">
            ...
            <div
                className={`absolute -inset-0.5 bg-gradient-to-r ${isPositive ? 'from-emerald-500 to-cyan-500' : isNegative ? 'from-red-500 to-orange-500' : 'from-amber-500 to-yellow-500'} rounded-2xl blur opacity-20 group-hover:opacity-40 transition duration-500`}
            ></div>
            <div className="relative bg-[#0F111A]/80 backdrop-blur-2xl rounded-2xl shadow-2xl p-4 sm:p-6 border border-white/10 ring-1 ring-white/5">
                {/* Header - Responsive */}
                ...
                <div className="space-y-3 sm:space-y-4">
                    ...
                    {/* Last Update */}
                    <div className="flex items-center gap-2 text-xs sm:text-sm text-slate-500 pt-2 sm:pt-3 border-t border-white/10">
                        <svg
