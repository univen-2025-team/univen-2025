...
export function BuyStockFeature({ data, onBack }: BuyStockFeatureProps) {
    ...
    useEffect(() => {
        if (quantity > 0 && estimatedCost > availableBalance) {
            setBalanceWarning(
                `⚠️ Số dư không đủ! Cần ${estimatedCost.toLocaleString(
                    'vi-VN'
                )} VND nhưng chỉ có ${availableBalance.toLocaleString('vi-VN')} VND`
            );
        } else {
            setBalanceWarning(null);
        }
    ...
    const handlePlaceOrder = async () => {
        ...
        if (estimatedCost > availableBalance) {
            setTransactionResult({
                success: false,
                message: `Số dư không đủ! Cần ${estimatedCost.toLocaleString(
                    'vi-VN'
                )} VND nhưng chỉ có ${availableBalance.toLocaleString('vi-VN')} VND.`
            });
            setStepIndex(3);
                                ...
                                <div className="space-y-4 rounded-lg border border-border/60 bg-muted/10 p-4">
                                    ...
                                    <div className="rounded-lg border border-slate-200 bg-white p-4 text-sm">
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            >
                                                {(availableBalance - estimatedCost).toLocaleString(
                                                    'vi-VN'
                                                )}{' '}
                                                VND
                                            </span>
